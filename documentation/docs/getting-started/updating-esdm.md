...
The check fetches a **small static file** and nothing else. There's no analytics call, no machine identifier, no schema or model contents – the request looks like any other HTTP `GET` against a public URL. The response payload is a JSON document of the form `{"version": "0.15.0"}`.
