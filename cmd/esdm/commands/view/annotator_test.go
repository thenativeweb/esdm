...
func TestAnnotate(t *testing.T) {
	t.Run("attaches matching severity to a node by exact location", func(t *testing.T) {
		root := &tree.Node{
			Kind:     "domain",
			Name:     "sample",
			Location: tree.SourceLocation{File: "domain.esdm.yaml", Line: 3, Column: 7},
		}
		...
		assert.Equal(t, tree.SeverityWarning, root.Severity)
	})
	...
	t.Run("bubbles a child's severity up to its ancestors", func(t *testing.T) {
		child := &tree.Node{
			Kind:     "aggregate",
			Name:     "widget",
			Location: tree.SourceLocation{File: "widget.esdm.yaml", Line: 5, Column: 7},
		}
		root := &tree.Node{
			Kind:     "domain",
			Name:     "sample",
		...
		assert.Equal(t, tree.SeverityError, child.Severity)
		assert.Equal(t, tree.SeverityError, root.Severity)
	})
	...
	t.Run("error severity wins over warning when both target the same node", func(t *testing.T) {
		root := &tree.Node{
			Kind:     "domain",
			Name:     "sample",
			Location: tree.SourceLocation{File: "f.esdm.yaml", Line: 1, Column: 1},
		}
		view.Annotate(root, []diag.Diagnostic{
	...
	t.Run("falls back to the deepest enclosing node when no exact match exists", func(t *testing.T) {
		...
		agg := &tree.Node{
			Kind:     "aggregate",
			Name:     "widget",
			Location: tree.SourceLocation{File: "agg.esdm.yaml", Line: 5, Column: 7},
		}
		domain.Children = []*tree.Node{agg}
		...
		assert.Equal(t, tree.SeverityError, agg.Severity)
		assert.Equal(t, tree.SeverityError, domain.Severity)
	})
}
